...
?>
                        ...
                        <div class="col-md-6 offset-3 pt-5">
                            ...
                            <div class="table-responsive table-responsive-data2">
                                <table class="table table-data2 text-center">
                                    ...
                                    <tbody>
                                        <?php
                                        $totals = mysqli_query($con, "SELECT SUM(pendaftar) AS total FROM pendaftar2022_2023") or die(mysqli_error($con));
                                        $total = mysqli_fetch_array($totals);
                                        $no = 1;
                                        $sqls = mysqli_query($con, "SELECT * FROM pendaftar2022_2023 a INNER JOIN bulan b ON a.kd_bulan = b.kd_bulan") or die(mysqli_error($con));
                                        foreach ($sqls as $sql) { ?>
                                            <tr class="spacer"></tr>
                                            <tr class="tr-shadow">
                                                <td><?= $no ?></td>
    ...
    if (isset($_POST['proses'])) {
        ...
        mysqli_query($con, "DELETE FROM acak2022_2023") or die(mysqli_error($con));
        ...
        for ($i = 1; $i <= $jumlah; $i++) {
            ...
            mysqli_query($con, "INSERT INTO acak2022_2023 ( kd_bulan, bilangan_acak ) VALUES ('$kode_bulan', '$random_number')") or die(mysqli_error($con));
        }
